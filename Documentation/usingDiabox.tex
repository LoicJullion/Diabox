...
The hydrographic sections need to be converted into Matlab format first. The hydrographic sections are formatted using  \verb"prepctd.m". In the directory that was created for the inversion, run:
\begin{tabular}{l}
\verb">>prepctd('filedir','section_name','ctdtemp','ctdsal','ctdpres','bin_press','lat','lon','dens_choice');"\\
\end{tabular}
 
This program performs several tasks:
\begin{enumerate}
  \item Check consistency of the input data
  \item Calculate the area of the bottom triangles
  \item Calculate the pressure of the isopycnal surfaces chosen. So far, only two choices: Neutral density and potential density. This part of the code needs to be updated according to the user's choice of isopycnal surfaces.
  \item Calculates geostrophic velocity referenced to the surface between  stations to deepest common depth.
  \item Adds the geostrophic velocity to area beneath deepest common depth. 
  \item Extrapolate temperature and salinity down into the bottom triangle
  \item Calculate salinity anomalies
\end{enumerate}
...
\section{Inverse model construction using DIABOX}
	...
	At this point, you should know which hydrographic sections and continents will define your closed boxes. You have prepared the sections using \verb"prepctd.m". The sections are run through the step 1 of DIABOX (\verb"dosection.m"). During this step, the CTD sections (2db grid) are used to calculate the variables necessary to construct the inverse model. Inverse models are usually isopycnic models: The fluxes are calculated in density layers and the different variables need to be mapped in density space. We use neutral density \cite{Jackett:1997p370} or potential density as our vertical coordinate and a layer of the model is defined by two neutral density contours.The code asks you the name of the section you want to process. Diabox is set up so that it will look for the sections in the \verb"test_inversion/section/" subdirectory. Just enter the name of the section:
...
Note that the in the script, the bathymetry is a 2*2deg version of Gebco that works well for large oceanic boxes but need to be updated if one wants more precise bathymetry (the file to be updated is makemap.m).
