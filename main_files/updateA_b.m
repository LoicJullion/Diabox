...
for k=1:nboxes;
  ...
  A(indb(2:nlayers-1,3,k),indF(1:nlayers-2,1,k))= ...
	.5*diag(ones(nlayers-2,1));
  A(indb(2:nlayers-1,3,k),indF(2:nlayers-1,1,k))= ...
        .5*diag(ones(nlayers-2,1));
  ...
  A(indb(2:nlayers-1,1,k),indF(1:nlayers-2,2,k))= ...
        .5*diag(ones(nlayers-2,1));
  A(indb(2:nlayers-1,1,k),indF(2:nlayers-1,2,k))= ...
        .5*diag(ones(nlayers-2,1));
  ...
  Tbar=interp1(glayers,fillnans(SSValue(:,3,k),0), ...
	glevels,'linear');
  Sbar=interp1(glayers,fillnans(SSValue(:,2,k),0), ...
	glevels,'linear');
  ...
  for i=1:nlayers-1;	% remove flux at glevel(i)
    for propnum=1:nproperties;
      A(indb(i,propnum,k),indF(i,1,k))= ...
        A(indb(i,propnum,k),indF(i,1,k)) ...
	-Fcorr_multfact(i,1,k).* ...
	meanprop_nonans(i,propnum,k);
      A(indb(i,propnum,k),indF(i,2,k))= ...
	A(indb(i,propnum,k),indF(i,2,k)) ...
	-Fcorr_multfact(i,2,k).* ...
	meanprop_nonans(i,propnum,k);
    end;
  end;
  for i=2:nlayers;	% add flux at glevel(i-1)
    for propnum=1:nproperties;
      A(indb(i,propnum,k),indF(i-1,1,k))= ...
	A(indb(i,propnum,k),indF(i-1,1,k)) ...
        +Fcorr_multfact(i-1,1,k).* ...
	meanprop_nonans(i-1,propnum,k);
      A(indb(i,propnum,k),indF(i-1,2,k))= ...
	A(indb(i,propnum,k),indF(i-1,2,k)) ...
        +Fcorr_multfact(i-1,2,k).* ...
	meanprop_nonans(i-1,propnum,k);
    end;
  end;
...
for sectnum=1:nsections;
  for i=1:nlayers+1;
    for j=1:nproperties;
      for k=1:nboxes;
	if (i<nlayers+1)
          A(indb(i,j,k),indE(sectnum))= ...
	    EkmanE(i,j,k,sectnum);
        else
          A(indb(i,j,k),indE(sectnum))= ...
            nansum(EkmanE(:,j,k,sectnum));
        end;
      end;
    end;
  end;
